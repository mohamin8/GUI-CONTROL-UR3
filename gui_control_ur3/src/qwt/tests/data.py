...
class DataPlot(QwtPlot):
    ...
    def timerEvent(self, e):
        ...
        # self.z[-1] = 0.5
        print(self.z)
        print(len(self.z), self.z[-1])


        # self.curveR.setData(self.x, self.y)
        # self.curveL.setData(self.x, self.z)
        if len(self.z)>=200:
            self.curveR.setData(self.x[-200:],self.z[-200:])
        else:
            self.curveR.setData(self.x,self.z)
        self.replot()
        self.phase += np.pi * 0.02
        self.z.append(0.8 - (2.0 * self.phase / np.pi) + 0.4 * random.random())
